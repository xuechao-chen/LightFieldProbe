/**
  \file data-files/shader/BumpMap/BumpMap.glsl
...
void bumpMapWelsh04(in sampler2D normalBumpMap, in float bumpMapScale, in float bumpMapBias, in vec2 texCoord, in vec3 tan_X, in vec3 tan_Y, in vec3 tan_Z, in float backside, in vec3 tsE, out vec3 wsN, out vec2 offsetTexCoord, out vec3 tsN, out float rawNormalLength, int maxIterations) {
    ...
    float bump = (texture(normalBumpMap, texCoord).w - 0.5 + bumpMapBias) * bumpMapScale;
...
void bumpMapTatarchuk06(in sampler2D normalBumpMap, in float bumpMapScale, in float bumpMapBias, in vec2 texCoord, in vec3 tan_X, in vec3 tan_Y, in vec3 tan_Z, in float backside, in vec3 tsE, out vec3 wsN, out vec2 offsetTexCoord, out vec3 tsN, out float rawNormalLength, int maxIterations) {
    ...
    float numSteps = mix(max(float(maxIterations) * bumpMapScale * 2.0, MIN_STEPS), MIN_STEPS,  max(0.0, (tsE.z - 0.4) / 0.6) );
    vec3 tsStep;
    ...
    vec4 NB = texture(normalBumpMap, offsetCoord.xy);
    ...
    while (NB.w < offsetCoord.z) {
        offsetCoord += tsStep;
        NB = texture(normalBumpMap, offsetCoord.xy);
    }

    // We've overshot slightly.  Assume that the surface is piecewise linear ala Policarpo and Oliveira '06, Tatarchuk '06
    //
    ...
    // the inner loop)
    vec4 prevNB = texture(normalBumpMap, offsetCoord.xy - tsStep.xy);
...
void bumpMap(in sampler2D normalBumpMap, in float bumpMapScale, in float bumpMapBias, in vec2 texCoord, in vec3 tan_X, in vec3 tan_Y, in vec3 tan_Z, in float backside, in vec3 tsE, out vec3 wsN, out vec2 offsetTexCoord, out vec3 tsN, out float rawNormalLength, const in int maxIterations) {  
	// This branch happens at compile time
	if (maxIterations == 0) {
        bumpMapBlinn78(normalBumpMap, bumpMapScale, bumpMapBias, texCoord, tan_X, tan_Y, tan_Z, backside, tsE, wsN, offsetTexCoord, tsN, rawNormalLength, maxIterations);   
	} else if (maxIterations == 1) {
        bumpMapWelsh04(normalBumpMap, bumpMapScale, bumpMapBias, texCoord, tan_X, tan_Y, tan_Z, backside, tsE, wsN, offsetTexCoord, tsN, rawNormalLength, maxIterations);
	} else {
		bumpMapTatarchuk06(normalBumpMap, bumpMapScale, bumpMapBias, texCoord, tan_X, tan_Y, tan_Z, backside, tsE, wsN, offsetTexCoord, tsN, rawNormalLength, maxIterations);
	}
}
