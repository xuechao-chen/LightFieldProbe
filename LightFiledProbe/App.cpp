...
void App::onInit()
{
	...
	m_pCubemapColor = Texture::createEmpty("Cubemap", 1024, 1024, ImageFormat::RGB32F(), Texture::DIM_CUBE_MAP);
	m_pCubemapDistance = Texture::createEmpty("CubemapDistance", 1024, 1024, ImageFormat::RGB32F(), Texture::DIM_CUBE_MAP);
	...
	for (int i = 0; i < 2; ++i)
	{
		Array<shared_ptr<Texture>> CubemapTextureColor, CubemapTextureDistance;

		for (int Face = 0; Face < 6; ++Face)
		{
			CubemapTextureColor.push(Texture::createEmpty("Cubemap" + Face, 1024, 1024, ImageFormat::RGB32F(), Texture::DIM_2D));
			CubemapTextureDistance.push(Texture::createEmpty("CubemapDistance" + Face, 1024, 1024, ImageFormat::RGB32F(), Texture::DIM_2D));
		}

		__renderLightFiledProbe(40 + i, CubemapTextureColor, CubemapTextureDistance);

		for (int Face = 0; Face < 6; ++Face)
		{
			Texture::copy(CubemapTextureColor[Face], m_pCubemapColor, 0, 0, 1.0f, Vector2int16(0, 0), CubeFace::POS_X, static_cast<CubeFace>(Face));
			Texture::copy(CubemapTextureDistance[Face], m_pCubemapDistance, 0, 0, 1.0f, Vector2int16(0, 0), CubeFace::POS_X, static_cast<CubeFace>(Face));
		}
		...
		renderDevice->push2D(m_LightFiledFB); {
			...
			args.setUniform("ProbeColorCubemap", m_pCubemapColor, Sampler);
...
void App::onGraphics3D(RenderDevice* rd, Array<shared_ptr<Surface> >& surface)
{
	...
	rd->push2D(); {
		...
		Args args;
		args.setRect(rd->viewport());
		args.setUniform("ProbeColorCubemap", m_pCubemapDistance, Sampler);
		args.setUniform("NumSamples", 2048);
		args.setUniform("LobeSize", 1.0f);
		args.setUniform("SphereSampler", m_SphereSamplerTexture, Sampler::buffer());
...
void App::__renderLightFiledProbe(uint32 vProbeIndex, Array<shared_ptr<Texture>>& vRadianceTexture, Array<shared_ptr<Texture>>& vDistanceTexture)
{
	...
	for (int face = 0; face < 6; ++face) {
		...
		Texture::copy(m_gbuffer->texture(GBuffer::Field::CS_POSITION), vDistanceTexture[face], 0, 0, 1,
					  Vector2int16((m_osWindowHDRFramebuffer->texture(0)->vector2Bounds() - vDistanceTexture[face]->vector2Bounds()) / 2.0f),
					  CubeFace::POS_X, CubeFace::POS_X, nullptr, false);
